package org.mvplugins.multiverse.inventoriesimporter.multiverseInventoriesImporter.migrate.worldinventories;
...
final class WorldInventoriesImportHelper {
    ...
    void importData() throws DataImportException {
        ...
        if (!wiGroups.isEmpty()) {
            ...
            if (defaultWorldGroup != null) {
                ...
                InvLogging.info("Removed automatically created world group in favor of imported groups.");
            }
        }
        ...
        InvLogging.info("Processing data for " + offlinePlayers.length + " players.  The larger than number, the longer"
                + " this process will take.  Please be patient. :)  Your server will freeze for the duration.");
        int playerCount = 0;
        for (OfflinePlayer player : offlinePlayers) {
            playerCount++;
            InvLogging.finer("(" + playerCount + "/" + offlinePlayers.length
            ...
            for (Group wiGroup : wiGroups) {
                ...
                if (worldGroup == null) {
                    InvLogging.finest("Could not import player data for WorldInventories group: " + wiGroup.getName()
    ...
    private void createGroups(List<Group> wiGroups) {
        for (Group wiGroup : wiGroups) {
            if (wiGroup.getWorlds().isEmpty()) {
                InvLogging.warning("Group '" + wiGroup.getName() + "' has no worlds."
            ...
            try {
                ...
                InvLogging.warning("Group '" + wiGroup.getName() + "' unable to import fully, sharing only inventory.");
                newGroup.getShares().setSharing(Sharables.ALL_INVENTORY, true);
            } catch (Error e) {
                InvLogging.warning("Group '" + wiGroup.getName() + "' unable to import fully, sharing only inventory.");
                newGroup.getShares().setSharing(Sharables.ALL_INVENTORY, true);
            ...
            InvLogging.info("Created Multiverse-Inventories group: " + wiGroup.getName());
        }
    }
    ...
    private Set<ProfileContainer> getWorldsWithoutGroups() {
        ...
        for (World world : Bukkit.getWorlds()) {
            if (worldGroupManager.getGroupsForWorld(world.getName()).isEmpty()) {
                InvLogging.fine("Added ungrouped world for importing.");
    ...
    private void transferData(OfflinePlayer player, Group wiGroup, ProfileContainer profileContainer) {
        ...
        InvLogging.finest("Player's data imported successfully for group: " + profileContainer.getContainerName());
    }
